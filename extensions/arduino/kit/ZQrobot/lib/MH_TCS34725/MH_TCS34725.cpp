...
float powf1(const float x, const float y)
{
...
uint16_t MH_TCS34725::getRGBZQ(uint8_t k) {
  ...
  if(k>3)
  {
    if(k==4) {return calculateColorTemperature(red, green, blue);}
    else{ return calculateLux(red, green, blue); }
  }
  else
  {
    uint32_t sum = clear;
    float r, g, b;
    if(k==1) { r = red; r /= sum; r *= 256; return gammatable[(int)r]; }
    if(k==2) { g = green; g /= sum; g *= 256; return gammatable[(int)g]; }
    if(k==3) { b = blue; b /= sum; b *= 256; return gammatable[(int)b]; }
  }
    //return calculateColorTemperature(red, green, blue);
}
